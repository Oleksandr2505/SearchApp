...
#include<thread>
    ...
    //включаємо під-дерикторії де хочемо знайти файл
    ...
    const fs::path file_name = "OpenSSH-Win64.zip";
    mutex mutex_process;
    ...
    {
        int num = 0;
        //рахууєм кількість файлів в данній директорії
    ...
    {
        //проходимся ітератором директорією
        using recursive_directory_iterator = std::filesystem::recursive_directory_iterator;

        auto d = fs::directory_iterator(wayToFile1);
       //через лямбду і count if проходимсяпо всім файлам і робим перевірку чи воно співпадає з нашим файлом
        ...
        //перевірка на те чи знайшов файл данний потік
        if (found != end(d))
        {
    ...
    //в інших функці те саме повторюється
    }
